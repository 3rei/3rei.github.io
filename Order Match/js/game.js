import { Customer } from './Order Match/js/customer.js';

// Global variables and constants
const canvas = document.getElementById('gameCanvas');
const ctx = canvas.getContext('2d');
const backgroundImage = new Image();
const orderBackgroundImage = new Image();
...
const items = [
  { name: 'mcspicy', src: './Order Match/image/mcspicy.png', x: 400, y: 450, width: 100, height: 100 },
  { name: 'fries', src: './Order Match/image/fries.png', x: 600, y: 450, width: 100, height: 100 },
  { name: 'drink', src: './Order Match/image/drink.png', x: 400, y: 300, width: 100, height: 100 },
  { name: 'icecream', src: './Order Match/image/icecream.png', x: 600, y: 300, width: 100, height: 100 },
];

// Image loading
submitButton.image.src = './Order Match/image/buttonup.png';
backgroundImage.src = './Order Match/image/bg.png';
fullHeartImage.src = './Order Match/image/heart.png';
brokenHeartImage.src = './Order Match/image/brokenheart.png'; 
orderBackgroundImage.src = './Order Match/image/orderBG.png';
...
function drawItems() {
  ...
  const orderX = 520;
  const orderY = 68;
  const orderItemSize = 40;
  const orderItemSpacing = 10;
  const itemsPerColumn = 3;

  for (let i = 0; i < currentOrder.length; i++) {
    const item = items.find(item => item.name === currentOrder[i]);
    if (item) {
      const column = Math.floor(i / itemsPerColumn);
      const row = i % itemsPerColumn;
      const itemX = orderX + column * (orderItemSize + orderItemSpacing);
      const itemY = orderY + row * (orderItemSize + orderItemSpacing);
      ctx.drawImage(item.image, itemX, itemY, orderItemSize, orderItemSize);
    }
  }
