...
function drawItems() {
  ...
  if (currentCustomer.image.complete && currentCustomer.image.naturalHeight !== 0) {
    ctx.drawImage(currentCustomer.image, 300 + currentCustomer.xOffset, 68 + currentCustomer.heightOffset, currentCustomer.imageWidth, currentCustomer.imageHeight);
  } else {
  ...
  ctx.drawImage(trayImage, -60, 150, 440, 440);
  ...
  // Calculate the width and height of the order background
  const orderWidth = Math.ceil(currentOrder.length / itemsPerColumn) * (orderItemSize + orderItemSpacing) - orderItemSpacing;
  const orderHeight = Math.min(currentOrder.length, itemsPerColumn) * (orderItemSize + orderItemSpacing) - orderItemSpacing;

  // Draw the white background for the order
  ctx.fillStyle = 'brown';
